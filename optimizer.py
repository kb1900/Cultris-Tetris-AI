...
class Optimizer():
	...
	def best_move(field,tetromino,next_tetromino,n):
		...
		if len(all_boards_first) > 4:				# for now this ensures that getting the best move doesnt take longer than 0.15s or so
		...
		all_boards_first.sort(key = lambda x: x[-1]) #sort by minimum second piece placed board score
