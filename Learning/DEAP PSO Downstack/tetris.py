	...
	def run_game(n,render=False):
		...
		while exit == 1:
			try:
				...
				## GET BEST MOVE ##
				t0 = time.time()
				best_drop = Optimizer.best_move(field,current_tetromino,next_tetromino,n)
				t1 = time.time()
				...
				if render == True:
					if piece_count %1 == 0:
						...
						print('Current Piece:', sequence[piece_count], 'NEW Best Rotation:', rotation, 'NEW Best Column:', column)
						print('Next Piece:', sequence[piece_count+1])
						print('piece_count:',piece_count,'clears_count:',clears_count)
						heuristics = field.heuristics()
						print('gaps', heuristics[0], 'bumpiness', heuristics[1], 'bog1', heuristics[2], 'bog2', heuristics[3], 'tall_holes', heuristics[4], 'max_bumps', heuristics[5],'stack gaps', heuristics[6], 'stack height', heuristics[7], 'sum_bumps_above_two', heuristics[8], 'trans_above_gap1',heuristics[9])
						# input()
...
if __name__ == '__main__':
	...
	try:
		...
		'''
		n = [17.266573527809562, 2.777217126349192, 6.760730777087559, 0.7876033208193283, 12.351036669926016, 2.9693729446011448, 17.853166241417732, 8.531717290316418, 1.5111635889673647, 4.507103638484812]
