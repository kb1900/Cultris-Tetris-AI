...
def several_generations(epochs):
	...
	for epoch in range(epochs):
		...
		print('Computing fitness for Epoch', epoch+1, 'took', float((time.time() - start_time)/60), 'minutes')
		...
		data = [[epoch+1, avg_score, median_score, avg_top_5, min_score, sorted_population[:5], float((time.time() - start_time)/60)]]
		temp_df = pd.DataFrame(data,columns=['Epoch','Average Score','Median Score', 'Top 5 Average score', 'Minimum Score', 'Top 5 Chromosomes', 'Evaluation Time'])
		...
		regression = Regression.linear_regression(x = df['Epoch'], y = df['Median Score'],ind = ind)
